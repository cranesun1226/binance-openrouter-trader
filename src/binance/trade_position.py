...
def adjust_qty_for_symbol(
    ...
    *,
    ...
    if min_qty > 0 and adjusted < min_qty:
        # Entry sizing can disable min bumping so a 100% allocation never exceeds the
        # requested notional just to satisfy the exchange's minimum symbol quantity.
        if not enforce_min_qty:
            return None
        adjusted = min_qty
...
def _reduce_entry_qty_for_retry(symbol: str, qty: Decimal) -> Optional[Decimal]:
    ...
    reduced_qty = adjust_qty_for_symbol(
        symbol,
        qty * ENTRY_ORDER_RETRY_QTY_FACTOR,
        enforce_min_qty=False,
    )
    ...
    if step > 0:
        reduced_qty = adjust_qty_for_symbol(symbol, qty - step, enforce_min_qty=False)
        if reduced_qty is not None and Decimal("0") < reduced_qty < qty:
            return reduced_qty
    return None
...
def get_account_overview(api_key: str, api_secret: str) -> Optional[Dict[str, float]]:
    payload = _signed_get_json(
        "/fapi/v3/account",
        api_key=api_key,
        api_secret=api_secret,
        params=[],
        operation_name="account_overview",
    )
    if not isinstance(payload, dict):
        return None
    ...
    available_balance = safe_float(payload.get("availableBalance"), 0.0)
    wallet_balance = safe_float(payload.get("totalWalletBalance"), 0.0)
    unrealized_profit = safe_float(payload.get("totalUnrealizedProfit"), 0.0)
    if equity <= 0.0:
        return None
    return {
        "equity": equity,
...
def get_account_equity(api_key: str, api_secret: str) -> Optional[float]:
    overview = get_account_overview(api_key, api_secret)
    if not isinstance(overview, dict):
        return None
    equity = safe_float(overview.get("equity"), 0.0)
...
def place_market_entry_order(
    ...
    *,
    ...
    adjusted_qty = adjust_qty_for_symbol(
        normalized_symbol,
        safe_decimal(qty),
        enforce_min_qty=False,
    )
...
def cancel_all_symbol_orders(api_key: str, api_secret: str, symbol: str) -> bool:
    """Cancel standard orders plus conditional stop orders for a symbol."""
    standard_ok = cancel_all_orders(api_key, api_secret, symbol)
    stop_ok = _cancel_stop_orders(api_key, api_secret, symbol)
    return bool(standard_ok and stop_ok)
