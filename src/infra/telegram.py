...
def send_telegram_photo(photo_bytes: bytes, *, filename: str = "chart.png", caption: str = "") -> bool:
    """Send a PNG image to Telegram with an optional HTML caption."""
    bot_token = load_env_var("TELEGRAM_BOT_TOKEN")
    chat_id = load_env_var("TELEGRAM_CHAT_ID")
    if not bot_token or not chat_id:
        logger.info("Telegram is not configured; skipping photo notification")
        return False
    if not photo_bytes:
        logger.info("Telegram photo payload is empty; skipping photo notification")
        return False

    url = f"{_TELEGRAM_API_BASE_URL}/bot{bot_token}/sendPhoto"
    data = {"chat_id": chat_id}
    normalized_caption = str(caption or "").strip()
    if normalized_caption:
        # Telegram supports the same HTML parse mode for photo captions.
        data["parse_mode"] = "HTML"
        data["caption"] = normalized_caption

    logger.info(
        "Sending Telegram photo | %s",
        format_log_details(
            {
                "chat_id": chat_id,
                "filename": filename,
                "bytes": len(photo_bytes),
                "caption_chars": len(normalized_caption),
            }
        ),
    )
    try:
        response = requests.post(
            url,
            data=data,
            files={"photo": (filename, BytesIO(photo_bytes), "image/png")},
            timeout=20,
        )
        payload = response.json()
    except Exception as exc:
    ...
    if response.status_code >= 400 or not isinstance(payload, dict) or not bool(payload.get("ok")):
        logger.warning(
            "Telegram API rejected photo | status=%s payload=%s",
            response.status_code,
            payload,
        )
        return False
