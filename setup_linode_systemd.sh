...
SERVICE_NAME="binance-openrouter-trader"
APP_USER="ubuntu"
APP_GROUP="ubuntu"
EXPECTED_APP_DIR="/home/ubuntu/binance-openrouter-trader"
...
Installs the Binance OpenRouter Trader scheduler as a systemd service.
Expected repository path: /home/ubuntu/binance-openrouter-trader
...
require_app_user() {
  # The scheduler runs as the normal Ubuntu login user so logs, db artifacts, and .env remain editor-accessible.
  if ! id "$APP_USER" >/dev/null 2>&1; then
    fail "Required service user '$APP_USER' does not exist on this VPS."
  fi

  if ! getent group "$APP_GROUP" >/dev/null 2>&1; then
    fail "Required service group '$APP_GROUP' does not exist on this VPS."
  fi
}
...
run_as_app_user() {
  # Keep generated files writable by ubuntu even though the installer itself runs with sudo.
  runuser -u "$APP_USER" -- "$@"
}

prepare_app_ownership() {
  log "Ensuring $APP_DIR is owned by $APP_USER:$APP_GROUP..."
...
ensure_virtualenv() {
  ...
  if ! run_as_app_user "$PYTHON_BIN" -m venv "$VENV_DIR"; then
    ensure_matching_venv_package
...
install_python_dependencies() {
  ...
  run_as_app_user "$VENV_DIR/bin/pip" install -r "$REQUIREMENTS_FILE"
}

prepare_runtime_dirs() {
  log "Ensuring runtime directories exist..."
  install -d -o "$APP_USER" -g "$APP_GROUP" "$APP_DIR/log" "$APP_DIR/db"
}
...
prepare_env_file() {
  if [[ -f "$ENV_FILE" ]]; then
    ...
    install -m 0600 -o "$APP_USER" -g "$APP_GROUP" "$ENV_EXAMPLE_FILE" "$ENV_FILE"
  else
...
required_env_ready() {
  ...
  for key in BINANCE_API_KEY BINANCE_API_SECRET OPENROUTER_API_KEY; do
